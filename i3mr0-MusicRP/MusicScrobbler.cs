...
namespace i3mr0_MusicRP {
    ...
    internal abstract class MusicScrobbler<C> where C : IScrobblerCredentials {
        ...
        public MusicScrobbler(string serviceName, string region, Logger? logger = null) {
        ...
        public async void Scrobbleit(AppleMusicInfo info) {
            ...
            try {
                ...
                var webScraper = new MusicWebScraper(info.SongName, info.SongAlbum, info.SongArtist, region);
    ...
    internal class MusicLastFmScrobbler : MusicScrobbler<LastFmCredentials> {
        private LastAuth? lastfmAuth;
        private IScrobbler? lastFmScrobbler;
        private ITrackApi? trackApi;

        public MusicLastFmScrobbler(string region, Logger? logger = null) : base("Last.FM", region, logger) { }
    ...
    internal class MusicListenBrainzScrobbler : MusicScrobbler<ListenBrainzCredentials> {
        private ListenBrainz? listenBrainzClient;

        public MusicListenBrainzScrobbler(string region, Logger? logger = null) : base("ListenBrainz", region, logger) { }
