...
internal class MusicDiscordClient {
    public enum RPSubtitleDisplayOptions {
        ArtistAlbum = 0, ArtistOnly = 1, AlbumOnly = 2, CustomFormat = 3
    }
    ...
    public string customFormat = "{artist} - {album}"; // Custom format for subtitle
    public bool showListeningTime = true; // Show how long you've been listening
    public bool showSongProgress = true; // Show progress bar in Discord

    public MusicDiscordClient(string discordClientID, bool enabled = true, RPSubtitleDisplayOptions subtitleOptions = RPSubtitleDisplayOptions.ArtistAlbum, Logger? logger = null) {
    ...
    public void SetPresence(AppleMusicInfo amInfo, bool showSmallImage, bool showBigImage) {
        ...
        switch (subtitleOptions) {
            ...
            case RPSubtitleDisplayOptions.AlbumOnly:
                subtitle = songAlbum;
                break;
            case RPSubtitleDisplayOptions.CustomFormat:
                subtitle = customFormat
                    .Replace("{artist}", songArtist)
                    .Replace("{album}", songAlbum)
