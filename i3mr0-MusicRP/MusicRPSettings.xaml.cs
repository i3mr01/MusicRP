...
namespace i3mr0_MusicRP {
    /// <summary>
    /// Interaction logic for MusicRPSettings.xaml
    /// </summary>
    public partial class MusicRPSettings : Window {
        private bool amRegionValid { 
            get { return Constants.ValidAppleMusicRegions.Contains(AppleMusicRegion.Text.ToLower()); }
        }
        public MusicRPSettings() {
            InitializeComponent();
            TextBlock_VersionString.Text = Constants.ProgramVersion;
            AppleMusicRegion.Text = i3mr0_MusicRP.Properties.Settings.Default.AppleMusicRegion;
        ...
        private void CheckBox_ClassicalComposerAsArtist_Click(object sender, RoutedEventArgs e) {
            ((MusicRPApp)Application.Current).UpdateScraperPreferences(CheckBox_ClassicalComposerAsArtist.IsChecked == true);
            SaveSettings();
        }
        ...
        private void CheckBox_ShowListeningTime_Click(object sender, RoutedEventArgs e) {
            SaveSettings();
        }

        private void CheckBox_ShowSongProgress_Click(object sender, RoutedEventArgs e) {
            SaveSettings();
        }

        private void ComboBox_RPSubtitleChoice_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            var newOption = MusicDiscordClient.SubtitleOptionFromIndex(ComboBox_RPSubtitleChoice.SelectedIndex);
            ((MusicRPApp)Application.Current).UpdateRPSubtitleDisplay(newOption);
            SaveSettings();
        }
        ...
        private void UpdateAppleMusicRegion() {
            if (amRegionValid) {
                ...
                i3mr0_MusicRP.Properties.Settings.Default.AppleMusicRegion = AppleMusicRegion.Text.ToLower();
            } else {
                AppleMusicRegion.Resources["TextControlFocusedBorderBrush"] = Brushes.Red;
                AppleMusicRegion.Text = i3mr0_MusicRP.Properties.Settings.Default.AppleMusicRegion;
            }
            SaveSettings();
            ((MusicRPApp)Application.Current).UpdateRegion();
        }
        private void ScrobbleMaxTime_TextChanged(object sender, TextChangedEventArgs e) {
            try {
                int.Parse(ScrobbleMaxTime.Text);
            } catch {
                ScrobbleMaxTime.Text = $"{i3mr0_MusicRP.Properties.Settings.Default.ScrobbleMaxWait}";
            } finally {
                SaveSettings();
            }
        }
        ...
        private async void SaveLastFMCreds_Click(object sender, RoutedEventArgs e) {
            ...
            if (i3mr0_MusicRP.Properties.Settings.Default.LastfmEnable) {
                // Signals the LastFM Scrobbler to re-init with new credentials
                var result = await ((MusicRPApp)Application.Current).UpdateLastfmCreds();
                if (result) {
            ...
            if (i3mr0_MusicRP.Properties.Settings.Default.ListenBrainzEnable) {
                // Signals the ListenBrainz Scrobbler to re-init with new credentials
                var result = await ((MusicRPApp)Application.Current).UpdateListenBrainzCreds();
                if (result) {
